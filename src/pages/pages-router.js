...
pagesRouter.route("/").get(requireAuth, (req, res, next) => {
...
pagesRouter.post("/", requireAuth, bodyParser, (req, res, next) => {
  ...
  PagesService.insertPage(req.app.get("db"), newPage)
    .then((page) => {
      ...
      res.status(201).location(`/page/${page.id}`).json(serializePage(page));
    })
    .catch(next);
});
